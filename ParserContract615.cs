...
{
    public class ParserContract615 : Parser
    {
        protected DataTable DtRegion;
        public readonly string[] ExceptFile = new String[0];

        public ParserContract615(string arg) : base(arg)
        {
        }
        ...
        public void ParsingXml(string f, string region)
        {
            ...
            if (fileInf.Exists)
            {
                using (var sr = new StreamReader(f, Encoding.Default))
                {
                    ...
                    if (f.Contains("pprf615Contract_"))
                    {
                        var p = new Contract615(json, f, region);
                        p.Work615();
                    }
                    else if (f.Contains("pprf615ContractProcedure_"))
                    {
                        var p = new ContractProcedure615(json, f, region);
                        p.Work615();
                    }
                }
            }
